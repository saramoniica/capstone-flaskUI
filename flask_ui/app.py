...
def index():
    ...
    stats = {
        ...
        'rev_table' : playstore.sort_values(by='Reviews', ascending=False).head(10).reset_index()[['Category','App','Reviews','Rating']].groupby('Reviews').head().to_html(classes=['table thead-light table-striped table-bordered table-hover table-sm'])
    }
    ...
    new = pd.crosstab(index = df2['Category'],columns = df2['Genres'],values= df2['Installs'],aggfunc='sum').melt().sort_values(by='value', ascending=False).head(5)
    X = new['Genres']
    Y = (new['value']/1000000).values
    my_colors= 'rgbkymc'
    fig = plt.figure(figsize=(8,4),dpi=300)
    fig.add_subplot()
    plt.bar(X,Y, color=my_colors)
    plt.ylabel('Installed in Million ')
    plt.xlabel('Genres')
